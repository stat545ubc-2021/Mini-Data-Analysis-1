...
I decided to reorder the review level from highest review to lowest review level. This allows for a better compassion of amount discounted between rating level to show the gradual increase in review levels. I used fct_relevel to turn the review level into factor and manual change the ordering as shown in the review_levels vector. 
...
games_achievement_fct <- games_achievement %>%
  mutate(genre = fct_lump_min(genre, 1000)) %>%
...
For the box-plot above, I decided to group genres with less than 1000 games into an other category. This is a much better solution that the box-plot in 1.1.2 as it doesn't hide any information and allows for a better compassion with the rest of the data set. I used the fct_lump_min function from the forecat library to lump all genres with less than 1000 games into the other group. Moreover, I grouped all positive review level into positive and all negative review level into negative review as I think that there was too much information displayed in the graph in 1.1.2. The plot above shows that negatively reviewed games tend to have lower achievement number compared to mixed and positive games between all genres. 
...
<!-------------------------- Start your work below ---------------------------->
```{R}
aov.data <- aov(achievements ~ review + genre, data=games_achievement_fct)
...
```{R}
achievement_summary <- games_achievement_fct %>%
  group_by(review) %>%
  summarize(mean_achievements = mean(achievements), 
            ...
            n = n())
write.csv(achievement_summary, here::here('output', 'game_achievement_summary.csv'))
read.csv(here::here('output','game_achievement_summary.csv'))
```
...
<!-------------------------- Start your work below ---------------------------->
```{R}
saveRDS(aov.data, here::here('output','aov_achievement_review_model'))
readRDS(here::here('output','aov_achievement_review_model'))
